    ...
    $.fn.InitUploader = function (b) {
        var fun = function (parentObj) {
            ...
            uploader.on('error', function (type) {
                switch (type) {
                    case 'Q_EXCEED_NUM_LIMIT':
                        $.messager.alert("系统提示", "上传文件数量过多");
                        break;
                    case 'Q_EXCEED_SIZE_LIMIT':
                        $.messager.alert("系统提示", "文件总大小超出限制");
                        break;
                    case 'F_EXCEED_SIZE':
                        $.messager.alert("系统提示", "文件大小超出限制");
                        break;
                    case 'Q_TYPE_DENIED':
                        $.messager.alert("系统提示", "禁止上传非"+p.filetypes+"格式文件");
                        break;
                    case 'F_DUPLICATE':
                        $.messager.alert("系统提示", "请勿重复上传该文件");
                        break;
                    default:
                        $.messager.alert("系统提示", type);
                        break;
                }
            });
            ...
            uploader.on('uploadError', function (file, reason) {
                ...
                $.messager.alert("系统提示", "错误代码：" + reason);
            });
            ...
            uploader.on('uploadSuccess', function (file, data) {
                ...
                if (data.code === 0) {
                    if (!p.multiple) {
                        parentObj.siblings(".upload-name").val(data.data.name);
                        parentObj.siblings(".upload-path").textbox("setValue", data.data.path);
                        parentObj.siblings(".upload-size").val(data.data.size);
                    } 
