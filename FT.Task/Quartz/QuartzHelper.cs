...
namespace FT.Task.Quartz
{
    ...
    /// 任务处理帮助类
    /// </summary>
    public class QuartzHelper : BaseJob
    {
        ...
        /// 任务程序集根目录
        /// </summary>
        private static string TASK_ROOT_PATH = IoHelper.GetMapPath("Task");
        ...
        public static void StartScheduler()
        {
            try
            {
                if (!scheduler.IsStarted)
                {
                    ...
                    scheduler.Start();
                    //获取所有执行中的任务
                    var tasks = QueryDb((context) =>
                    {
                        return
                            context.SystemTask.Where(s => s.Status == 0)
                                .FromCache(CachePolicy.WithSlidingExpiration(TimeSpan.FromSeconds(15)))
                                .Select(s => new TaskUtil
                                {
        ...
        public static void DeleteJob(string JobKey)
        {
            ...
            if (scheduler.CheckExists(jk))
            {
                ...
                Log.Debug(string.Format("任务“{0}”已经删除", JobKey));
            }
        }
        ...
        public static void ScheduleJob(TaskUtil taskUtil, bool isDeleteOldTask = false)
        {
            ...
            if (ValidExpression(taskUtil.CronExpressionString))
            {
                ...
                var trigger = new CronTriggerImpl
                {
                    ...
                    Name = taskUtil.TaskID.ToString(),
                    Description = taskUtil.TaskName
                };
                scheduler.ScheduleJob(job, trigger);
                if (taskUtil.Status == 0)
                {
                    var jk = new JobKey(taskUtil.TaskID.ToString());
                    scheduler.PauseJob(jk);
                }
                else
                {
                    Log.Debug(string.Format("任务“{0}”启动成功,未来5次运行时间如下:", taskUtil.TaskName));
                    var list = GetTaskeFireTime(taskUtil.CronExpressionString, 5);
        ...
        public static void PauseJob(string JobKey)
        {
            ...
            if (scheduler.CheckExists(jk))
            {
                ...
                Log.Debug(string.Format("任务“{0}”已经暂停", JobKey));
            }
        }
        ...
        public static void ResumeJob(string JobKey)
        {
            ...
            if (scheduler.CheckExists(jk))
            {
                ...
                Log.Debug(string.Format("任务“{0}”恢复运行", JobKey));
            }
        }
        ...
        public static void RunOnceTask(string JobKey)
        {
            ...
            if (scheduler.CheckExists(jk))
            {
                ...
                Log.Debug(string.Format("任务“{0}”立即运行", JobKey));
            }
        }
        /// 获取类的属性、方法  
        /// </summary>  
